...
def get_box3d_corners_helper(centers, headings, sizes):
    ...
    print(headings.size(), c.size(), zeros.size(), s.size())
...
def get_box3d_corners(center, heading_residual, size_residual):
    ...
    mean_sizes = torch.from_numpy(g_mean_size_arr).float().view(1,NUM_SIZE_CLUSTER,3)+ size_residual #.cuda()
                  #.cuda() # (1,8,3)+(bs,8,3) = (bs,8,3)
...
class FrustumPointNetLoss(nn.Module):
    ...
    def forward(self, logits, mask_label, \
        ...
        heading_class_loss = F.nll_loss(F.log_softmax(heading_scores,dim=1), \
                                        heading_class_label.long().squeeze()) #tensor(2.4505, grad_fn=<NllLossBackward>)
        ...
        size_class_loss = F.nll_loss(F.log_softmax(size_scores,dim=1),\
                    size_class_label.long().squeeze())#tensor(2.0240, grad_fn=<NllLossBackward>)
        ...
        predicted_size_residual_normalized_dist = torch.sum( \
            size_residual_normalized * scls_onehot_repeat, dim=1)#32,3 .cuda()
        mean_size_arr_expand = torch.from_numpy(g_mean_size_arr).float().view(1, NUM_SIZE_CLUSTER, 3)  #.cuda() \
             # 1,8,3
